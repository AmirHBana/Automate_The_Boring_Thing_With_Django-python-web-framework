...
def scrape_stock_data(symbol, exchange):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            price_changed = soup.find(f"fin-streamer",

                                      {"data-symbol": {symbol}, "data-test": "qsp-price-change"}).span.text

            percentage_changed = soup.find(f"fin-streamer", {"data-symbol": {symbol},
                                                             "data-field": "regularMarketChangePercent"}).span.text
